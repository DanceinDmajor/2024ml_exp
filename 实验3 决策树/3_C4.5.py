...
def information_gain_ratio(X_feature, y):
    total_entropy = entropy(y)
    values, counts = np.unique(X_feature, return_counts=True)
    weighted_entropy = sum((counts[i] / sum(counts)) * entropy(y[X_feature == values[i]]) for i in range(len(values)))
    intrinsic_value = -sum((counts[i] / sum(counts)) * log2(counts[i] / sum(counts)) for i in range(len(values)))
...
class DecisionTreeC45:
    ...
    def _find_best_split(self, X, y):
        ...
        for feature in X.columns:
            gain_ratio = information_gain_ratio(X[feature], y)
    ...
    def _predict_row(self, row):
        ...
        while isinstance(node, dict):
            ...
            node = node[feature].get(row[feature], y_train.mode()[0])
        return node
...
# 训练决策树模型
model = DecisionTreeC45(max_depth=3)
